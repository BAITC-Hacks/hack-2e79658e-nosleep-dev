...
> [!NOTE]
> Текущее состояние: контракт, датасет и формула Score зафиксированы и перепроверены независимым скриптом. Движок расчёта (`internal/scoring`) и AI-объяснение (`internal/llm`) реализованы и слиты в `main`. HTTP-обвязка и веб-интерфейс — в работе, см. [`.ai/STATUS.md`](.ai/STATUS.md).
...
| **Сравнение сценариев** *(в работе)* | 2–3 набора решений бок о бок, с AI-комментарием, что и почему отличается |
| **Потолок возможного** *(в работе)* | «Ваш результат — это X% от лучшего из всех допустимых наборов» |
...
> [!IMPORTANT]
> HTTP-обвязка (`apps/api/cmd`) и веб-приложение ещё собираются. Команды ниже — целевой сценарий, который проверяется командой к фиче-фризу T+4:15. До этого момента статус — в [`.ai/STATUS.md`](.ai/STATUS.md).

```bash
git clone https://github.com/BAITC-Hacks/hack-2e79658e-nosleep-dev
cd hack-2e79658e-nosleep-dev
cp .env.example .env
make dev            # apps/api на :8000, apps/web на :3000
...
cd apps/api && go test ./internal/...     # golden-тесты движка и LLM-клиента
```
...
- [ ] HTTP-обвязка `internal/http` и `cmd/server`
- [ ] Веб-интерфейс: выбор решений, live-карта, результат
- [ ] Живая геокарта районов (с откатом на схематичную SVG)
- [ ] `/optimum` и `/compare`
- [ ] Городские события `/events/draw`
